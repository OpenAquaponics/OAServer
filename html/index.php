...
abstract class RestApiInterface extends Singleton {
    ...
    abstract public function all($class);
...
class OAUserInfo extends RestApiInterface {
    public function all($class) {
        return $this->db->all('SELECT * FROM ' . $class);
    }
...
$app->map('/:user(/:class)(/:id)', function($user, $class, $id = null){
    $app = Slim::getInstance();
    try {
...
//        if(in_array($method, array('POST', 'PUT', 'DELETE')) && !$auth) throw new ForbiddenException();
        ...
        if($method == 'GET' && $id == null) $res = $class->all($class_str);
/*
